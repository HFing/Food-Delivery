...
const Checkout = ({ route, navigation }) => {
    ...
    useEffect(() => {
        ...
        console.log('User Token:', user?.token); // Log the user token to ensure it's being retrieved correctly
    }, [store, quantities, total, user?.token]);
    ...
    const handleConfirmOrder = async () => {
        ...
        try {
            const response = await authApis(user?.token).post(endpoints.createOrder, orderData);
